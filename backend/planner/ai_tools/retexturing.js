...
const WEDDING_THEMES = {
    'haldi': {
        ...
        negativePrompt: 'dark colors, dull lighting, western decorations, modern furniture, minimal setup, cold colors, formal chairs, contemporary style'
    },
    'mehendi': {
        ...
        negativePrompt: 'formal setup, dark colors, western style, minimal decorations, modern furniture, harsh lighting, masculine colors'
    },
    'sangeet': {
        ...
        negativePrompt: 'minimal lighting, quiet setup, formal arrangements, dull colors, static decorations, poor acoustics'
    },
    'wedding': {
        ...
        negativePrompt: 'simple setup, modern style, minimal decorations, western wedding elements, contemporary furniture, non-traditional colors'
    },
    'reception': {
        ...
        negativePrompt: 'casual setup, minimal decorations, poor lighting, basic furniture, informal arrangements, dull atmosphere'
    },
    'engagement': {
        ...
        negativePrompt: 'overly formal, dark colors, minimal romance, harsh lighting, impersonal setup'
    }
};

// FLUX API functions
async function callFluxAPIFixed(imageBuffer, prompt, negativePrompt, modelType = 'flux-kontext-pro', imageCount = 1) {
    try {
        if (!isAIEnabled()) {
            throw new Error('AI service is not configured. Please set BFL_API_KEY in environment variables.');
        }

        console.log(`🚀 Starting FLUX API call with model: ${modelType}, imageCount: ${imageCount}`);
        ...
        if (imageCount > 1) {
            ...
            for (let i = 0; i < imageCount; i++) {
                const singleResult = await callFluxAPIFixed(imageBuffer, prompt, negativePrompt, modelType, 1);
        ...
        if (!response.ok) {
            ...
            console.error(`❌ FLUX API Error ${response.status}:`, errorText.substring(0, 200));
            
            // Try fallback model if primary fails
            if (response.status === 403 || response.status === 422) {
                const fallbackModel = modelType === 'flux-kontext-pro' ? 'flux-kontext-max' : 'flux-kontext-pro';
                console.log(`🔄 Trying fallback model: ${fallbackModel}`);
                
                return await callFluxAPIFixed(imageBuffer, prompt, negativePrompt, fallbackModel);
            }
            
            throw new Error(`FLUX API error: ${response.status} - ${errorText.substring(0, 100)}`);
        }
        ...
        return {
            ...
            seed: result.result?.seed || result.seed || Math.floor(Math.random() * 1000000),
...
async function pollForResult(taskId, customPollingUrl = null) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        try {
            ...
            const pollingUrl = customPollingUrl || `${process.env.FLUX_POLLING_ENDPOINT || 'https://api.bfl.ai/v1/get_result'}?id=${taskId}`;
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.log(`⚠️ Poll attempt ${attempt + 1} - Status: ${response.status}`);
            ...
            if (result.status === 'Error') {
                const errorDetails = result.details || result.error || 'API processing error';
...
router.get('/health', (req, res) => {
    ...
    res.json({
        ...
        bfl_api_key: isAIEnabled() ? 'configured' : 'not configured',
        credit_costs: {
...
router.post('/check-credits', protect, async (req, res) => {
    try {
        const { operation = "generate", imageCount = 1, modelType = 'flux-kontext-pro', theme } = req.body;
        ...
        const perImageCost = (theme && WEDDING_THEMES[theme] && WEDDING_THEMES[theme].creditCost) 
            ? WEDDING_THEMES[theme].creditCost 
            : (modelType === 'flux-pro-1.1' ? 15 : 10);
    ...
    try {
        ...
        if (!isAIEnabled()) {
            return res.status(503).json({
                success: false,
                error: 'AI service is not configured. Please set BFL_API_KEY in environment variables.',
                instruction: 'Get your API key from https://api.bfl.ai and add it to your .env file as BFL_API_KEY=your_key_here'
            });
        }
        ...
        const { theme, customPrompt = '', modelType = 'flux-kontext-pro', imageCount = 1 } = req.body;
        ...
        const validImageCount = [1, 4].includes(parseInt(imageCount)) ? parseInt(imageCount) : 1;

        // Calculate credit cost
        const perImageCost = (theme && WEDDING_THEMES[theme] && WEDDING_THEMES[theme].creditCost) ? WEDDING_THEMES[theme].creditCost : (modelType === 'flux-pro-1.1' ? 15 : 10);
        ...
        let finalPrompt = '';
        let negativePrompt = 'blurry, low quality, distorted, ugly, bad anatomy, dark lighting, poor composition, cluttered space, messy arrangement';
        ...
        finalPrompt += '. Ensure professional photography quality, excellent lighting, vibrant colors, clean and organized layout, beautiful symmetry, and stunning visual appeal';
        ...
        try {
            ...
            if (result && result.url) {
                ...
                try {
                    ...
                    user.deductCredits(creditsNeeded, `AI generation - ${theme || 'custom'}`, 'ai_generation', {
                        theme: theme || 'custom',
                        modelType,
                        imageCount: validImageCount,
                        generationId: result.generationId
                    ...
                    console.error('Failed to deduct credits after generation:', deductErr);
                    responseData.creditWarning = 'Generation succeeded but failed to deduct credits. Please contact support.';
                }
        ...
        } else if (error.message.includes('rate') || error.message.includes('busy')) {
            errorMessage = 'Service is busy. Please wait and try again.';
            statusCode = 429;
        } else if (error.message.includes('API processing error')) {
            errorMessage = 'AI service encountered an issue. Please try with a different image or simpler prompt.';
        ...
        res.status(statusCode).json({
            success: false,
            error: errorMessage,
            code: error.message.includes('credits') ? 'INSUFFICIENT_CREDITS' : 'GENERATION_ERROR'
        });
    }
});
...
router.post('/feedback', async (req, res) => {
    try {
        const { feedback, resultUrl, theme, customPrompt, modelType, creditCost, userId } = req.body;
