...
const stepsMap = [
  { label: "Bid Placed", key: "pending", description: "Finding the perfect planners for your vision..." },
  { label: "Vendor Review", key: "viewed", description: "Planners are reviewing your request and moodboard." },
  { label: "Quotation Received", key: "quoted", description: "Quotes are ready for you to compare and accept." },
  { label: "Booking Complete", key: "accepted", description: "Your planner is confirmed and booking is complete." },
];
...
const CoupleBidPlaced = () => {
  ...
  useEffect(() => {
    const fetchLatestQuote = async () => {
      ...
      try {
        ...
        setError(err.response?.data?.error || "Failed to load your bid details.");
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    socket.on("quote_update", (data) => {
      if (data.quoteId === quote._id) {
        quoteAPI.getById(quote._id).then((res) => {
  ...
  const progressMeta = useMemo(() => {
    ...
    const completed = stepsMap.filter((step) => getStepStatus(quote.status, step.key) === "done").length;
    const activeStep = stepsMap.find((step) => getStepStatus(quote.status, step.key) === "active") || stepsMap[0];
    ...
    <main className="min-h-screen bg-[#0d0907] px-4 py-8 text-white md:px-8">
      <div className="mx-auto max-w-[1280px]">
        <div className="mb-6 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={() => navigate(`/couple/bid-dashboard/${quote._id}`, { state: { justSubmitted } })}
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_360px]">
          <section className="rounded-[34px] border border-white/10 bg-[#1a1411]/92 p-7 shadow-[0_0_0_1px_rgba(255,255,255,0.03)] md:p-10">
            <div className="mb-8 flex items-center justify-between">
              <div>
                <p className="text-[11px] uppercase tracking-[0.28em] text-[#bdaea4]">Bid Journey</p>
                ...
                {stepsMap.map((step) => {
                  ...
                  return (
                    <div
                      key={step.key}
                      onClick={() => {
                        if (step.key === "pending" || step.key === "viewed" || step.key === "quoted" || step.key === "accepted") {
                          navigate(`/couple/bid-dashboard/${quote._id}`, {
                            state: { activePanel: step.key === "quoted" ? "proposals" : step.key === "accepted" ? "messages" : "overview" },
                          });
                        }
                      }}
                    ...
                    >
                      ...
                      <div className="pt-1">
                        <h2 className={`text-[22px] font-semibold ${isActive ? "text-[#f1ccd8]" : isDone ? "text-white/80" : "text-white/30"}`}>
                          {step.label}
                        </h2>
                        <p className={`mt-2 max-w-xl text-sm ${isActive ? "text-white/50" : "text-white/22"}`}>
                          {step.description}
                        </p>

                        {isActive && step.key === "quoted" && quote.responses?.length > 0 && (
                          <div className="mt-4 flex flex-wrap gap-3">
                            <button
                              type="button"
                              onClick={(event) => {
                                event.stopPropagation();
                                navigate(`/couple/bid-dashboard/${quote._id}`, { state: { activePanel: "proposals" } });
                              }}
                              className="rounded-full bg-[#f0d0bd] px-4 py-2 text-xs font-semibold text-[#1d1714]"
                            >
                              Review {quote.responses.length} Proposal{quote.responses.length > 1 ? "s" : ""}
                            </button>
                            <button
                              type="button"
                              onClick={(event) => {
                                event.stopPropagation();
                                navigate(`/couple/bid-dashboard/${quote._id}`, { state: { activePanel: "messages" } });
                              }}
                        ...
                        {isActive && step.key === "pending" && (
                          <div className="mt-4 rounded-2xl border border-[#f0d0bd]/10 bg-[#211917] px-4 py-3 text-xs text-white/45">
                            Planners usually start reviewing within a few hours. Your request ID is{" "}
                            <span className="font-semibold text-[#f0d0bd]">{quote._id.slice(-6).toUpperCase()}</span>.
                          </div>
                        )}

                        {!isActive && !isDone && (
                          <div className="mt-4 rounded-2xl border border-white/5 bg-white/[0.02] px-4 py-3 text-xs text-white/28">
                            This stage unlocks automatically when planners update your bid. It is not a clickable next page.
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div className="mt-8 rounded-[28px] border border-white/10 bg-[#16110e] p-5">
              <p className="text-[11px] uppercase tracking-[0.22em] text-white/35">What You Can Do Now</p>
              <div className="mt-4 flex flex-wrap gap-3">
                <button
                  type="button"
                  onClick={() => navigate(`/couple/bid-dashboard/${quote._id}`, { state: { activePanel: "overview" } })}
                  className="rounded-full bg-[#f0d0bd] px-4 py-2 text-xs font-semibold text-[#1d1714]"
                >
                  Open Bid Dashboard
                </button>
                <button
                  type="button"
                  onClick={() => navigate(`/couple/bid-dashboard/${quote._id}`, { state: { activePanel: "messages" } })}
          ...
          <aside className="space-y-6">
            <div>
              <p className="mb-3 text-[11px] uppercase tracking-[0.24em] text-white/18">Request Overview</p>
              <div className="rounded-[28px] border border-white/10 bg-[#1a1411]/92 p-6">
                <div className="relative overflow-hidden rounded-[22px] border border-white/10 bg-[#2a231e] p-5">
                  ...
                  <div className="absolute right-9 top-9 rounded-2xl border border-white/10 bg-[#5c544f]/95 px-4 py-2 text-sm font-semibold text-white shadow-xl">
                    {quote.images?.length || 0} Vision{quote.images?.length === 1 ? "" : "s"}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-[28px] border border-white/10 bg-[#1a1411]/92 px-7 py-6">
              <div className="space-y-7">
                <div className="flex items-center justify-between border-b border-white/10 pb-5">
                  <span className="text-[11px] uppercase tracking-[0.22em] text-white/35">Budget</span>
                  <span className="text-[18px] font-semibold text-white">{quote.eventDetails?.budget || "Flexible"}</span>
                </div>
                <div className="flex items-center justify-between border-b border-white/10 pb-5">
                  <span className="text-[11px] uppercase tracking-[0.22em] text-white/35">Location</span>
                  <span className="text-[18px] font-semibold text-white">{quote.eventDetails?.city || "TBD"}</span>
                </div>
                <div className="flex items-center justify-between border-b border-white/10 pb-5">
                  <span className="text-[11px] uppercase tracking-[0.22em] text-white/35">Guests</span>
                  <span className="text-[18px] font-semibold text-white">{quote.eventDetails?.guestCount || "TBD"}</span>
                </div>
                <div className="pt-1">
                  <span className="text-[11px] uppercase tracking-[0.22em] text-white/35">Any Special Notes</span>
            ...
            <div className="rounded-[28px] border border-white/10 bg-gradient-to-b from-[#1a1411]/92 to-[#15100d]/92 p-6 text-[14px] text-white/58">
              <div className="flex items-start gap-4">
                ...
                <p>
                  Your contact details remain hidden from planners until you choose to accept a proposal.
                </p>
              </div>
            </div>

            <div className="rounded-[28px] border border-white/10 bg-[#1a1411]/92 p-6">
              <p className="text-[11px] uppercase tracking-[0.22em] text-white/35">Next Actions</p>
              ...
              <p className="mt-4 text-xs text-white/28">
                Submitted on {formatDate(quote.createdAt)} • Wedding date {formatDate(quote.eventDetails?.weddingDate)}
              </p>
            </div>
          </aside>
        </div>
