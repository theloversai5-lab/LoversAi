    ...
    const fetchData = async () => {
      try {
        ...
        if (chatRes.success) {
          ...
          const rms = chatRes.rooms
            .filter((r) => r.lastMessage)
            .map((r) => {
              const other =
                r.participants?.find((p) => p._id !== currentUser.id) || {};
              let ts = 0;
              if (r.lastMessage && r.lastMessage.timestamp) {
                const p = Date.parse(r.lastMessage.timestamp);
                ts = Number.isFinite(p) ? p : 0;
              }
              return {
                id: r._id,
                roomId: r._id,
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2 text-white/70 text-xs tracking-wide justify-center md:justify-start">
              ...
              {wp.weddingDate && (
                <span className="flex items-center gap-2">
                  📅{" "}
                  {new Date(wp.weddingDate).toLocaleDateString("en-GB", {
                    day: "numeric",
                    month: "short",
                    year: "numeric",
                  })}
      ...
      <div className="max-w-6xl mx-auto px-6 pt-6">
        ...
        <div className="inline-flex bg-black/40 p-1 rounded-xl border border-white/10 mb-8 backdrop-blur-md">
          {navs.map((nav) => (
            ...
            >
              {nav.label}
              {nav.badge && (
                <span className="absolute top-1.5 right-1 w-4 h-4 bg-[#e6c6b2] text-black rounded-full text-[9px] flex items-center justify-center font-bold">
                  {nav.badge}
                </span>
              )}
            </button>
        ...
        {activeTab === "overview" && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4 flex-1">
                  <div className="flex items-center justify-between pb-4 border-b border-white/[0.04]">
                    <span className="flex items-center gap-2 text-sm text-white/70">
                      <span className="text-green-500">✓</span> Accepted
                    </span>
                  ...
                  <div className="flex items-center justify-between pb-4 border-b border-white/[0.04]">
                    ...
                    <span className="w-6 h-6 rounded-full bg-yellow-500/20 text-yellow-400 flex items-center justify-center text-xs">
                      {bidCounts.pending}
                    </span>
                  </div>
                  <div className="flex items-center justify-between pb-4 border-b border-white/[0.04]">
                    <span className="flex items-center gap-2 text-sm text-white/70">
                      <span className="text-red-500">✕</span> Rejected
                    </span>
              ...
              <div className="bg-black/40 backdrop-blur-md rounded-2xl border border-white/[0.08] p-6 flex flex-col text-white">
                ...
                <div className="space-y-5 flex-1">
                  ...
                  {messages.length === 0 && (
                    <p className="text-white/30 text-sm italic py-4">
                      No recent messages from planners.
                    </p>
                  )}
                </div>
                <button
            ...
            {/* Quick Actions */}
            <div className="mt-10">
              <h3 className="font-heading text-xl text-[#F9F7F5] mb-4 tracking-wide">
                Quick Actions
              </h3>
          ...
          <div className="space-y-4">
            ...
            {quotes.length === 0 ? (
              <p className="text-white/40">
                You have not submitted any visions for quoting yet.
              </p>
            ) : (
              quotes.map((q) => (
                <div
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm font-medium">
                        {q.eventDetails?.city || "Undecided Location"}
                      </p>
                      <p className="text-[11px] text-white/40 mt-1">
                        {q.responses?.length || 0} planner responses • Submitted{" "}
        ...
        {activeTab === "messages" && (
          <div className="space-y-4 max-w-3xl">
            ...
            {messages.length === 0 ? (
              <p className="text-white/40">
                No messages yet. Planners will contact you here once they bid.
              </p>
            ) : (
              messages.map((msg) => (
                <div
                  ...
                  onClick={() =>
                    msg.quoteId &&
                    navigate(`/couple/bid-dashboard/${msg.quoteId}`, {
                      state: { activePanel: "messages", roomId: msg.roomId },
                    })
                  }
            ...
            <div className="mt-8 p-4 border border-white/10 rounded-xl bg-white/[0.02]">
              <p className="text-xs text-white/40 text-center">
                Open any conversation above to continue inside the quote CRM.
                Accepted planners can continue requirements and finalization
                there.
              </p>
            </div>
          </div>
