...
const statusColor = {
  ...
  quoted: "bg-purple-500/15 text-purple-400 border border-purple-500/25",
  accepted: "bg-blue-500/15 text-blue-400 border border-blue-500/25",
...
export default function PlannerBids() {
  ...
  useEffect(() => {
          ...
          if (res.success && res.quotes?.length > 0) {
            setBids(
              res.quotes.map((quote) => ({
                _id: quote._id,
                couple: quote.couple?.fullName || "Couple",
                budget: quote.eventDetails?.budget || "Flexible",
                city: quote.eventDetails?.city || "N/A",
                religion: quote.eventDetails?.tradition || "N/A",
                prompt: quote.eventDetails?.notes || "",
    ...
    return () => {
      socket.off("new_bid", fetchBids);
      socket.off("quote_update", fetchBids);
      socket.disconnect();
    };
  }, []);
  ...
  return (
    <div className="space-y-5 animate-fadeInUp">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-2xl text-white">Planner Leads</h1>
          <p className="text-xs text-white/30 mt-1">
            New couple requests you can quote on
          </p>
        </div>
        <span className="badge-open text-xs px-3 py-1 rounded-full font-medium">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search by couple or city..."
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/5">
                <th className="text-left text-[11px] text-white/30 font-medium p-4 uppercase tracking-wider">
                  Couple
                </th>
                <th className="text-left text-[11px] text-white/30 font-medium p-4 uppercase tracking-wider hidden md:table-cell">
                  Budget
                </th>
                <th className="text-left text-[11px] text-white/30 font-medium p-4 uppercase tracking-wider hidden md:table-cell">
                  City
                </th>
                <th className="text-left text-[11px] text-white/30 font-medium p-4 uppercase tracking-wider hidden lg:table-cell">
                  Tradition
                </th>
                <th className="text-left text-[11px] text-white/30 font-medium p-4 uppercase tracking-wider hidden xl:table-cell">
                  Vision
                </th>
                <th className="text-left text-[11px] text-white/30 font-medium p-4 uppercase tracking-wider">
                  Status
                </th>
                <th className="text-right text-[11px] text-white/30 font-medium p-4 uppercase tracking-wider">
                  Action
                </th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                filtered.map((bid) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-sm text-white/60 hidden md:table-cell">
                      {bid.budget}
                    </td>
                    <td className="p-4 text-sm text-white/60 hidden md:table-cell">
                      {bid.city}
                    ...
                    <td className="p-4 hidden xl:table-cell">
                      {bid.images?.[0] && (
                        <img
                          src={bid.images[0]?.url || bid.images[0]}
                          alt="Moodboard vision"
                          className="w-16 h-10 rounded-md object-cover border border-white/10"
                        />
                      )}
                    </td>
                    <td className="p-4">
      ...
      {selectedBid && (
        ...
        >
          <div
            className="glass-card-strong rounded-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto"
          ...
          >
            ...
            <div className="p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {selectedBid.images.map((img, i) => (
                      <img
                        ...
                        src={img.url || img}
                        alt="Vision preview"
                        className="w-full h-28 rounded-lg object-cover border border-white/10"
                      />
                    ))}
                  </div>
                </div>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  disabled
                  className="flex-1 loverai-btn-outline text-sm py-2.5 rounded-xl flex items-center justify-center gap-2 opacity-50 cursor-not-allowed"
                >
                  <svg
                    className="w-4 h-4"
                    fill="none"
