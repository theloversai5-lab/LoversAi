...
export default function Signup() {
  ...
  const handleGoogleSignupRedirect = (user, isNewUser) => {
    ...
    if (resolvedRole === "planner") {
      navigate(isNewUser ? "/planner/onboarding" : "/planner", {
        replace: true,
      });
      return;
    }
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      if (!firebaseIdToken) {
        throw new Error(
          "Firebase did not return a sign-up credential. Please try again.",
        );
      }

      const data = await firebaseLogin(firebaseIdToken, role.toLowerCase());
      ...
      if (actualRole !== expectedRole) {
        setError(
          `This Google account is already registered as ${actualRole}. Please continue with the matching account type.`,
        );
        logout();
        return;
      }
          ...
          <div className="grid min-h-[620px] md:grid-cols-[1.1fr_0.92fr]">
            ...
            <div className="p-8 md:p-10 lg:p-12 flex items-center">
              <div className="w-full">
                <div className="mb-8">
                  <h2 className="text-3xl font-heading text-[#fff5ea]">Create Account</h2>
                  <p className="text-sm text-white/45 mt-1">Join the LoversAi experience</p>
                </div>
